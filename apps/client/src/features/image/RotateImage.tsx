...
export const RotateImage: React.FC = () => {
  ...
  const [downloadName, setDownloadName] = useState<string>("rotated-image");
  ...
  const handleRotate = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5000/api/v1/image/rotate", formData, {
      ...
      setDownloadName(`${originalName}-rotated.${format}`);
    } catch (err: any) {
        ...
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>Rotate an image clockwise or counter-clockwise.</CardDescription>
            </div>
          </div>
        </CardHeader>
          ...
          {!downloadUrl ? (
            <>
              ...
              <Button onClick={handleRotate} disabled={!file || isProcessing} className="w-full h-12 text-md">
                {isProcessing ? "Rotating..." : <><Play className="w-5 h-5 mr-2" /> Rotate Image</>}
              </Button>
            </>
          ) : (
            <div className="flex flex-col items-center justify-center p-12 border border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50 dark:bg-zinc-900/50 space-y-6">
              ...
              <div className="text-center">
                ...
                <p className="text-zinc-500 mt-2">Your rotated image is ready for download.</p>
              </div>
